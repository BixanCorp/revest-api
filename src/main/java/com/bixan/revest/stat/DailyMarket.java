...
public class DailyMarket {
	...
	private void loadAndParse() throws IOException {
		...
		// String csvPath = Configurator.getDataPath() + File.separatorChar +
		// this.dailyFileName;
		...
		try (InputStream csvStream = DailyMarket.class.getResourceAsStream(this.dailyFileName)) {
			...
			try {
				csv = CSVParser.parse(csvStream, Charset.forName("UTF-8"),
						CSVFormat.DEFAULT.withHeader().withAllowMissingColumnNames());
				// csv = CSVParser.parse(f, Charset.forName("UTF-8"),
				// CSVFormat.DEFAULT.withHeader().withAllowMissingColumnNames());
			} catch (IOException e) {
	...
	private void doSampling() {
		...
		for (int i = 0; i < sampleSize; i++) {
			...
			sample[i] = (prices[(length - bound) + idx] - prices[(length - bound) + idx - 1])
					/ prices[(length - bound) + idx - 1];
			/*
			 * sample[i] = (prices[bound - i] - prices[bound - i -1]) / prices[bound - i
			 * -1];
			 */
		}
	}
